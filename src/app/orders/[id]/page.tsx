...
export default function OrderDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const generateOrderPDF = () => {
    ...
    const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Order - ' + order.order_number + '</title><style>* { margin: 0; padding: 0; box-sizing: border-box; } body { font-family: "Segoe UI", Tahoma, Geneva, Verdana, sans-serif; padding: 40px; color: #1e293b; background: #fff; } .header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 30px; padding-bottom: 20px; border-bottom: 2px solid #6366f1; } .logo { font-size: 28px; font-weight: bold; color: #6366f1; } .logo-sub { font-size: 12px; color: #64748b; } .doc-info { text-align: right; } .doc-title { font-size: 24px; font-weight: bold; color: #1e293b; } .doc-number { font-size: 14px; color: #64748b; margin-top: 4px; } .section { margin-bottom: 25px; } .section-title { font-size: 14px; font-weight: 600; color: #6366f1; margin-bottom: 12px; padding-bottom: 6px; border-bottom: 1px solid #e2e8f0; } .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 15px; } .info-box { padding: 12px; background: #f8fafc; border-radius: 8px; } .info-label { font-size: 11px; color: #64748b; text-transform: uppercase; margin-bottom: 4px; } .info-value { font-size: 14px; font-weight: 500; color: #1e293b; } .workflow-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; } .workflow-item { padding: 10px; border-radius: 8px; text-align: center; } .workflow-complete { background: #dcfce7; border: 1px solid #22c55e; } .workflow-pending { background: #fef2f2; border: 1px solid #ef4444; } .workflow-label { font-size: 11px; font-weight: 600; } .workflow-status { font-size: 10px; margin-top: 4px; } table { width: 100%; border-collapse: collapse; margin-top: 10px; } th, td { padding: 10px; text-align: left; border-bottom: 1px solid #e2e8f0; font-size: 13px; } th { background: #f1f5f9; font-weight: 600; color: #475569; } .amount { text-align: right; } .total-row { font-weight: bold; background: #6366f1; color: white; } .total-row td { border: none; } .summary-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 15px; margin-top: 20px; } .summary-box { padding: 15px; border-radius: 8px; text-align: center; } .summary-total { background: linear-gradient(135deg, #6366f1, #8b5cf6); color: white; } .summary-paid { background: #dcfce7; } .summary-expense { background: #fef2f2; } .summary-profit { background: #dbeafe; } .summary-value { font-size: 18px; font-weight: bold; margin-top: 4px; } .summary-label { font-size: 11px; opacity: 0.8; } .footer { margin-top: 30px; padding-top: 15px; border-top: 1px solid #e2e8f0; font-size: 11px; color: #64748b; text-align: center; } @media print { body { padding: 20px; } }</style></head><body>' +
      '<div class="header"><div><div class="logo">Aura Knot</div><div class="logo-sub">Photography & Events</div></div><div class="doc-info"><div class="doc-title">ORDER DETAILS</div><div class="doc-number">' + order.order_number + '</div><div class="doc-number">Date: ' + formatDate(order.created_at) + '</div></div></div>' +
