...
function QuotationContent() {
    ...
    try {
      ...
      (items || []).forEach((item) => {
        if (item.category === "Photography") {
          ...
          // Extract last ' - ' segment as session if present
          let session = "";
          const sessionMatch = desc.match(/\s-\s([^\-]+)$/);
          if (sessionMatch) {
          ...
          // If the service name isn't in our known list, store it as 'Other' and keep the name in remarks
          const isKnown = additionalServices.includes(serviceName);
          additionalServicesArr.push({
            id: generateId(),
            service: isKnown ? serviceName : "Other",
            remarks: isKnown ? "" : serviceName,
            rate: item.total_price || 0,
            session: session,
  ...
  // Ensure the generated quotation number is unique in the DB (retry a few times)
  ...
  const ensureUniqueQuotationNumber = async (initialNumber: string) => {
    if (!supabase) return initialNumber;

    let candidate = initialNumber;
    for (let i = 0; i < 10; i++) {
      try {
        ...
        const match = candidate.match(/_(\d{4})$/);
        const nextNum = match ? parseInt(match[1]) + 1 : 1;
        candidate = candidate.replace(/_(\d{4})$/, `_${nextNum.toString().padStart(4, "0")}`);
      } catch (e) {
        // If check fails, return the original candidate to avoid blocking
        console.warn("Could not verify uniqueness for quotation number, proceeding with candidate:", candidate, e);
        return candidate;
      }
    }

    return candidate; // after retries
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (isEditMode && editQuotationId) {
        ...
        if (quotationError) {
          console.error(
            ...
            quotationError?.message ?? JSON.stringify(quotationError),
            quotationError?.code,
            quotationError?.details
          );

          // If unique constraint still triggers for any reason, attempt one more time with a new number
          const errMsg = (quotationError as { message?: string; code?: string; details?: string })?.message || "";
          if (/duplicate key|unique constraint/i.test(errMsg)) {
            console.warn("Detected duplicate key on quotation_number. Retrying generation and insert.");
            const retryNumber = await ensureUniqueQuotationNumber(await generateQuotationNumber());
            const { data: q2, error: q2Err } = await supabase
              .from("quotations")
              .insert({
                ...quotationData,
                quotation_number: retryNumber,
                status: "Draft",
                valid_until: createdValidUntil,
              })
              .select()
            ...
            if (q2Err) {
              console.error("Retry insert failed:", q2Err?.message ?? JSON.stringify(q2Err));
              throw q2Err;
            }
      ...
      console.error("Error saving quotation:", err.message ?? err.code ?? JSON.stringify(err));
      const errText = (err.message || err.details || "").toString();
      let errorMsg = err.message || err.details || `Failed to ${isEditMode ? "update" : "create"} quotation. Make sure the database schema is set up.`;

      if (/duplicate key|unique constraint/i.test(errText)) {
        errorMsg = "A quotation with the generated number already exists. Please try again or contact support.";
      }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {/* Total & Actions */}
      <div className="sticky bottom-0 z-30 -mx-6 bg-[var(--background)]/80 px-6 py-3 backdrop-blur-xl">
        <div className="flex flex-col gap-3 rounded-2xl border border-[var(--border)] bg-[var(--card)] px-4 py-3 shadow-lg sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:gap-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-2xl font-bold text-[var(--foreground)]">₹{totalAmount.toLocaleString()}</span>
            </div>
          </div>
